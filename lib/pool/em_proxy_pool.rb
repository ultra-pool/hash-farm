...
#
# The EmProxyPool connect to an other distant pool.
...
class EmProxyPool < Pool
  ...
  #
  # EmProxyPool.new( host, port, username, password )
  # EmProxyPool.new( uri )
  # EmProxyPool.new( url )
  #
  # EmProxyPool.new( "middlecoin.com", 3333, "barbu", "toto" )
  # EmProxyPool.new( "http://barbu:toto@middlecoin.com:3333" )
  # EmProxyPool.new( url: "http://barbu:toto@middlecoin.com:3333" )
  # EmProxyPool.new( url: "http://middlecoin.com:3333", user: "barbu", password: "toto" )
  # EmProxyPool.new( "http://middlecoin.com:3333", "barbu", "toto" )
  # EmProxyPool.new( URI("http://barbu:toto@middlecoin.com:3333") )
  # EmProxyPool.new( URI("http://middlecoin.com:3333"), "barbu", "toto" )
  #
  ...
  def init_proxy_listeners
    forward( @proxy, 'error' )
    on( 'error' ) do |error| EmProxyPool.log.error( error.to_s ) end
      ...
      if resp.result?
        ...
        if resp.result[1] != @extra_nonce_1 || resp.result[2] != @extra_nonce_2_size
          ...
          if ! @notifications.kind_of?( Array ) || @notifications.size != 2
            EmProxyPool.log.warn "[#{@host}] On subscribed, notifications = #{@notifications}"
          end
          ...
          EmProxyPool.log.verbose "[#{@host}] subscribe : extra1=%s, extra2size=%d" % [@extra_nonce_1, @extra_nonce_2_size]
          # On a reconnect.
  ...
  def stop
    EmProxyPool.log.info "[#{name}] Stopping..."
    return self if ! started?
  ...
  def notify_all_workers job=@last_job
    EmProxyPool.log.verbose "[#{@host}] Notify #{@workers.size} workers with job #{job.id}."
  ...
  def on_pool_set_difficulty diff
    ...
    EmProxyPool.log.verbose "[#{@host}] New difficulty received : #{@next_diff}."
  ...
  def on_pool_notify job
    EmProxyPool.log.verbose "[#{@host}] New job received : #{job}."
    job.pool = @name
  ...
  def subscribe worker
    EmProxyPool.log.debug "proxy pool subscribe"
  ...
  def submit worker, req
    ...
    # EmProxyPool.log.debug "pdiff for #{job_id} = #{pdiff}"
    if share.match_difficulty( pdiff )
      ...
      EmProxyPool.log.debug("[#{@name}] Send back pool job : #{pool_job}")
      @proxy.mining.submit( *pool_job ) do |resp|
        if resp.result?
          @accepted_shares += 1
          EmProxyPool.log.info "[#{@name}]#{job_id}/#{share.ident}@#{worker.name} Accepted by pool"
          share.pool_result = true
        else
          @rejected_shares += 1
          EmProxyPool.log.warn "[#{@name}]#{job_id}/#{share.ident}@#{worker.name} Not accepted by pool : #{resp.error}"
        ...
        if share.valid?
          ...
          EmProxyPool.log.error "[#{@name}] Share invalid before save ! #{share.to_json}"
        end
      end
    end
    ...
    EmProxyPool.log.warn "[#{@name}][#{worker.name}] Fail on submit : #{err}\n" + err.backtrace[0...5].join("\n")
    nil
  rescue => err
    EmProxyPool.log.error "[#{@name}] #{err}\n" + err.backtrace[0..5].join("\n")
    EmProxyPool.log.error "worker=#{worker.inspect}"
    EmProxyPool.log.error "share=#{share.inspect}"
    nil
  end
