...
class ShareToolTest < ActiveSupport::TestCase
  ...
  @ntime         = 1374999143.to_hex(4)
  @nbits         = "1b47c7ea"
  @nonce         = 1499913728.to_hex(4)
  ...
  @job = [
    "a",
    @previous_hash.reverse_hash_int,
    @coinb1,
    @coinb2,
    @merkle_branches.map { |h| h.reverse_hex },
    @version,
    @nbits,
    (@ntime.hex-10).to_hex(4)
  ]
  @submit = [@extra_nonce_2, @ntime, @nonce]

  @share = ShareTool.new( @worker, @job, @submit )
  end

  def test_initialize
  assert_kind_of Time, @share.created_at

  assert_equal @worker.name, @share.worker_name
  ...
  assert_equal @version.hex, @share.version
  assert_equal @nbits.hex, @share.nbits
  
  assert_equal @extra_nonce_2, @share.extra_nonce_2
  assert_equal @ntime.hex, @share.ntime
  assert_equal @ntime.hex - 10, @share.jtime
  assert_equal @nonce.hex, @share.nonce

  assert_equal @extra_nonce, @share.extra_nonce
  assert_kind_of String, @share.merkle_root
  end

  def test_extra_nonce
  assert_equal @extra_nonce, @share.extra_nonce
  end

  def test_coinbase_hex
  assert_equal @coinbase_hex, @share.coinbase_hex
  end

  def test_coinbase_hash
  ...
  def test_to_hex
  hex = @share.to_hex
  assert_equal @version.reverse_hex + @previous_hash.reverse_hex + @merkle_root.reverse_hex + @ntime.reverse_hex + @nbits.reverse_hex + @nonce.reverse_hex, hex

  version, previous_hash, merkle_root, ntime, nbits, nonce = *hex.to_bin.unpack("L<a32a32L<L<L<")
  assert_equal @version.hex, version
  assert_equal @previous_hash.reverse_hex, previous_hash.to_hex
  assert_equal @merkle_root.reverse_hex, merkle_root.to_hex
  assert_equal @ntime.hex, ntime
  assert_equal @nbits.hex, nbits
  assert_equal @nonce.hex, nonce
  end
  ...
  assert share.is_valid_share?
  # == share_target + 1
  share.stubs(to_hash: "00000000ffff0000000000000000000000000000000000000000000000000001")
  refute share.is_valid_share?
  ...
  worker = OpenStruct.new( name: "", extra_nonce_1: '', jobs_pdiff: {"a" => 1} )
